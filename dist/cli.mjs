...
function isDirectSkillUrl(input) {
	...
	if (input.includes("bitbucket.org/")) return false;
	return true;
}
...
function parseSource(input) {
	...
	if (bitbucketRepoMatch) {
		...
		return {
			type: "bitbucket",
			url: `https://bitbucket.org/${workspace}/${repo.replace(/\.git$/, "")}.git`
		};
	}
...
function getBitbucketAuthHeaders() {
	if (process.env.BITBUCKET_TOKEN) return { Authorization: `Bearer ${process.env.BITBUCKET_TOKEN}` };
	if (process.env.BITBUCKET_USERNAME && process.env.BITBUCKET_APP_PASSWORD) return { Authorization: `Basic ${Buffer.from(`${process.env.BITBUCKET_USERNAME}:${process.env.BITBUCKET_APP_PASSWORD}`).toString("base64")}` };
	return {};
}
async function fetchBitbucketSkillFolderHash(ownerRepo, skillPath, authHeaders) {
	...
	for (const branch of ["main", "master"]) try {
		...
		if (data.values && data.values.length > 0) return data.values[0].hash;
	} catch {
		continue;
	}
	return null;
...
async function handleRemoteSkill(source, url, options, spinner) {
	...
	if (successful.length > 0 && installGlobally) try {
		...
		if (remoteSkill.providerId === "github") {
			...
			const hash = await fetchBitbucketSkillFolderHash(remoteSkill.sourceIdentifier, url, getBitbucketAuthHeaders());
			if (hash) skillFolderHash = hash;
		}
			...
			for (const skill of selectedSkills) {
				...
				if (successfulSkillNames.has(skillDisplayName)) try {
					...
					if (parsed.type === "github" && skillPathValue) {
						...
						const hash = await fetchBitbucketSkillFolderHash(normalizedSource, skillPathValue, getBitbucketAuthHeaders());
						if (hash) skillFolderHash = hash;
					}
...
async function runCheck(args = []) {
	...
	for (const skillName of skillNames) {
		...
		if (!entry) continue;
		if (entry.sourceType !== "github" && entry.sourceType !== "bitbucket" || !entry.skillFolderHash || !entry.skillPath) {
			skippedCount++;
			continue;
		}
	...
	for (const [source, skills] of skillsBySource) for (const { name, entry } of skills) try {
		let latestHash = null;
		if (entry.sourceType === "github") latestHash = await fetchSkillFolderHash(source, entry.skillPath, token);
		else if (entry.sourceType === "bitbucket") latestHash = await fetchBitbucketSkillFolderHash(source, entry.skillPath, bbAuthHeaders);
		if (!latestHash) {
			errors.push({
				name,
				source,
				error: `Could not fetch from ${entry.sourceType}`
			});
			continue;
		}
...
async function runUpdate() {
	...
	for (const skillName of skillNames) {
		const entry = lock.skills[skillName];
		if (!entry) continue;
		if (entry.sourceType !== "github" && entry.sourceType !== "bitbucket" || !entry.skillFolderHash || !entry.skillPath) continue;
		checkedCount++;
		try {
			let latestHash = null;
			if (entry.sourceType === "github") latestHash = await fetchSkillFolderHash(entry.source, entry.skillPath, token);
			else if (entry.sourceType === "bitbucket") latestHash = await fetchBitbucketSkillFolderHash(entry.source, entry.skillPath, bbAuthHeaders);
	...
	for (const update of updates) {
		...
		if (update.entry.skillPath) {
			...
			if (update.entry.sourceType === "bitbucket") installUrl = `${installUrl}/src/main/${skillFolder}`;
			else installUrl = `${installUrl}/tree/main/${skillFolder}`;
		}
		if (spawnSync("npx", [
			"-y",
			"skills",
